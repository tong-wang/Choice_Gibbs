...
generate.profit <- function(row, scenario, num) {
    ...
    out1$profit <- (out1$price - Cost) * out1$demand
...
meanvar0 <- ddply(profit0, .(price), summarize, mean=mean(profit), var=var(profit))
meanvar1 <- ddply(profit1, .(price), summarize, mean=mean(profit), var=var(profit))
meanvar3.m <- ddply(profit3.m, .(price), summarize, mean=mean(profit), var=var(profit))
meanvar0c <- ddply(profit0c, .(price), summarize, mean=mean(profit), var=var(profit))
meanvar1c <- ddply(profit1c, .(price), summarize, mean=mean(profit), var=var(profit))
meanvar3c.m <- ddply(profit3c.m, .(price), summarize, mean=mean(profit), var=var(profit))
meanvar0.EM <- ddply(profit0.EM, .(price), summarize, mean=mean(profit), var=var(profit))
meanvar3c.m.EM <- ddply(profit3c.m.EM, .(price), summarize, mean=mean(profit), var=var(profit))
...
cmeanvar0 <- ddply(profit0, .(posterior.id, price), summarize, mean=mean(profit), var=var(profit))
#cmeanvar1 <- ddply(profit1, .(posterior.id, price), summarize, mean=mean(profit), var=var(profit))
#cmeanvar3.m <- ddply(profit3.m, .(posterior.id, price), summarize, mean=mean(profit), var=var(profit))
#cmeanvar0c <- ddply(profit0c, .(posterior.id, price), summarize, mean=mean(profit), var=var(profit))
cmeanvar1c <- ddply(profit1c, .(posterior.id, price), summarize, mean=mean(profit), var=var(profit))
cmeanvar3c.m <- ddply(profit3c.m, .(posterior.id, price), summarize, mean=mean(profit), var=var(profit))
#cmeanvar0.EM <- ddply(profit0.EM, .(posterior.id, price), summarize, mean=mean(profit), var=var(profit))
cmeanvar3c.m.EM <- ddply(profit3c.m.EM, .(posterior.id, price), summarize, mean=mean(profit), var=var(profit))


# variace of the conditional mean
var.cmean0 <- ddply(cmeanvar0, .(price), summarize, var=var(mean))
#var.cmean1 <- ddply(cmeanvar1, .(price), summarize, var=var(mean))
#var.cmean3.m <- ddply(cmeanvar3.m, .(price), summarize, var=var(mean))
#var.cmean0c <- ddply(cmeanvar0c, .(price), summarize, var=var(mean))
var.cmean1c <- ddply(cmeanvar1c, .(price), summarize, var=var(mean))
var.cmean3c.m <- ddply(cmeanvar3c.m, .(price), summarize, var=var(mean))
var.cmean3c.m.EM <- ddply(cmeanvar3c.m.EM, .(price), summarize, var=var(mean))
