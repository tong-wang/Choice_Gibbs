...
save(z1c, observation1c, file="MNL_Scenario1c.binary.L1.RData")
...
### save plots
require(ggplot2)
pdf('MNL_Scenario1c.binary.L1.lambda.pdf', width = 8, height = 8)
ggplot(data=data.frame(samples.lambda.truncated)) + geom_density(aes(x=samples.lambda.truncated), color="black")
dev.off()
pdf('MNL_Scenario1c.binary.L1.beta.pdf', width = 8, height = 8)
