...
MH.mvnorm <- function (logpost, start, sigma=diag(length(start)), scale=1, nrun=1000, ...) 
{
    dim = length(start)
...
discreteMH.norm <- function (logpost, start, scale=1, nrun=1000, ...) 
{
    dim = length(start)
    MC = array(0, c(nrun, dim))
    b1 = start
...
discreteMH.mvnorm <-function (logpost, start, sigma=diag(length(start)), scale=1, nrun=1000, ...) 
{
    dim = length(start)
    MC = array(0, c(nrun, dim))
    b1 = start
    ...
    for (i in 1:nrun) {
        ...
        b2 = as.vector(round(b1 + scale * a %*% rnorm(dim)))
